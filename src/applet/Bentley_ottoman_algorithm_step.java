 ...
 *
 */
public class Bentley_ottoman_algorithm_step {

	// 入力(左から右へのベクトルになるようにしておく).
	/** セグメント. */
	public ArrayList<Line2D> segmentList = new ArrayList<>();
	/**  hash.put(3.0, new ArrayList(Arrays.asList(0, 1))) , key: x軸座標の値， value:((0:交点,1:左端，2:右端), セグメントインデックス) */
	public HashMap<Double, ArrayList<Integer>> _segHashMap  = new HashMap<>();
	/** イベントキュー(segmentのpoinntが入る)(x軸で昇順ソートされる). */
	public LinkedList<Point2D> eventQueue = new LinkedList<>();
	/** sweep line(segmentListのインデックス)(y軸昇順ソートされている). 
	 * sweep lineは注目しているx軸の線上にあるセグメントのリスト
	 **/
	public ArrayList<Integer>sweepLine = new ArrayList<>();
	
	// 出力.
	/** intersection list(segmentListのインデックスが入る) */
	public ArrayList<Point2D>intersectionList = new ArrayList<>();
	
	MyPanel _samplePanel;
	public int[][] intersectionJudgeIndex = {{-1, -1},{-1, -1}};
	public boolean[] intersectionJudge = {false, false};

	public Bentley_ottoman_algorithm_step(MyPanel samplePanel) {
	 ...
	 * @param aSeg
	 */
	public void init(ArrayList<Line2D> aSeg){
		segmentList = aSeg;
		...
		if(!eventQueue.isEmpty()){
			...
			if(segE_whichEdge == 1){
				...
				if(sweepLineIndex > 0){	// sweep line へプッシュしたセグメントが先頭ではない.
					intersectionJudgeIndex[0] = new int[] {sweepLine.get(sweepLineIndex-1), sweepLine.get(sweepLineIndex)};
					if(isIntersect(
							segmentList.get(sweepLine.get(sweepLineIndex)), 
							segmentList.get(sweepLine.get(sweepLineIndex-1)))
					){// segEとsegBの交差判定.
						intersectionJudge[0] = true;
						// 交点をイベントキューへプッシュ.
						...
						//intersectionJudgeIndex[0] = new int[] {-1, -1};
						intersectionJudge[0] = false;
					}
				}else{
					intersectionJudgeIndex[0] = new int[] {-1, -1};
					intersectionJudge[0] = false;
				...
				if(sweepLineIndex < sweepLine.size()-1){	// sweep line へプッシュしたセグメントが語尾でない.
					intersectionJudgeIndex[1] = new int[] {sweepLine.get(sweepLineIndex), sweepLine.get(sweepLineIndex+1)};
					if(isIntersect(
							segmentList.get(sweepLine.get(sweepLineIndex)), 
							segmentList.get(sweepLine.get(sweepLineIndex+1)))
					){	// segEとsegAの交差判定.
						intersectionJudge[1] = true;
						// 交点をイベントキューへプッシュ.
						Point2D ip = calcIntersectionPoint(segmentList.get(sweepLine.get(sweepLineIndex)), segmentList.get(sweepLine.get(sweepLineIndex+1)));
						pushEventQueue(ip);
					}else{
						//intersectionJudgeIndex[1] = new int[] {-1, -1};
						intersectionJudge[1] = false;
					}
				}else{
					intersectionJudgeIndex[1] = new int[] {-1, -1};
					intersectionJudge[1] = false;
			...
			/////////////////////////
				...
				if(sweepLineIndex < sweepLine.size()-1){	// 語尾でない.
					intersectionJudgeIndex[0] = new int[] {sweepLine.get(sweepLineIndex), sweepLine.get(sweepLineIndex+1)};
					// 交差判定.
					if(isIntersect(
							segmentList.get(sweepLine.get(sweepLineIndex)),
							segmentList.get(sweepLine.get(sweepLineIndex+1)))
				...
				if(sweepLineIndex > 1){	// 先頭ではない.
					intersectionJudgeIndex[1] = new int[] {sweepLine.get(sweepLineIndex-1), sweepLine.get(sweepLineIndex-2)};
					// 交差判定.
					if(isIntersect(
							segmentList.get(sweepLine.get(sweepLineIndex-1)),
